...
def create_model(fingerprint_input, model_settings, model_architecture,
  ...
  """
  print("is_training is {}".format(is_training))
  if model_architecture == 'single_fc':
    return create_single_fc_model(fingerprint_input, model_settings,
                                  is_training)
  elif model_architecture == 'conv':  # CNN-1
    return create_conv_model(fingerprint_input, model_settings, is_training)
  elif model_architecture == 'low_latency_conv':  # CNN-2
    ...
    return create_dnn_model(fingerprint_input, model_settings, model_size_info,
      ...
      return create_cnn_model2(fingerprint_input, model_settings, model_size_info,
                               is_training)
  elif model_architecture == 'cnn3':
      return create_cnn_model3(fingerprint_input, model_settings, model_size_info,
                               is_training)
...
def create_cnn_model3(fingerprint_input, model_settings, model_size_info,
                      is_training):
    ...
    third_filter_count = model_size_info[10]
    third_filter_height = model_size_info[11]   #time axis
    third_filter_width = model_size_info[12]    #frequency axis
    third_filter_stride_y = model_size_info[13] #time axis
    third_filter_stride_x = model_size_info[14] #frequency_axis
    ...
    third_conv = tf.nn.conv2d(second_dropout, third_weights, [
        1, third_filter_stride_y, third_filter_stride_x, 1
    ], 'VALID') + third_bias
    ...
    third_conv_output_width = math.ceil(
        (second_conv_output_width - third_filter_width + 1) /
        third_filter_stride_x)
    third_conv_output_height = math.ceil(
        (second_conv_output_height - third_filter_height + 1) /
        third_filter_stride_y)
    third_conv_element_count = int(
        third_conv_output_width*third_conv_output_height*third_filter_count)
    flattened_third_conv = tf.reshape(third_dropout,
                                      [-1, third_conv_element_count])

    # linear layer
    W = tf.get_variable('W', shape=[third_conv_element_count, linear_layer_size],
                        initializer=tf.contrib.layers.xavier_initializer())
    b = tf.get_variable('b', shape=[linear_layer_size])
    flow = tf.matmul(flattened_third_conv, W) + b
    ...
    if is_training:
        ...
        return first_conv, second_conv, third_conv, flow, first_fc, final_fc
...
def create_cnn_model2(fingerprint_input, model_settings, model_size_info,
  ...
  ##### update 2020/05/21 #######
  # first_conv_bn = tf.layers.batch_normalization(first_conv, training=is_training,
  #                name='bn1')
  ...
  ##### update 2020/05/21 #######
  # second_conv_bn = tf.layers.batch_normalization(second_conv, training=is_training,
  #                 name='bn2')
  ...
  # ##### update 2020/05/21 #######
  # first_fc_bn = tf.layers.batch_normalization(first_fc, training=is_training,
  #              name='bn3')
        ...
        for layer_no in range(0,num_layers):
          if layer_no==0:
            net = slim.convolution2d(fingerprint_4d, conv_feat[layer_no],
                      ...
                      padding='VALID', scope='conv_1')
            result.append(net)
            net = tf.nn.relu(net)
            # net = slim.batch_norm(net, scope='conv_1/batch_norm')
          else:
            net, result = _depthwise_separable_conv(net, conv_feat[layer_no],
                      kernel_size = [conv_kt[layer_no],conv_kf[layer_no]],
                      stride = [conv_st[layer_no],conv_sf[layer_no]],
                      sc='conv_ds_'+str(layer_no), result=result)
          t_dim = math.ceil(t_dim/float(conv_st[layer_no]))
          f_dim = math.ceil(f_dim/float(conv_sf[layer_no]))

        net = slim.avg_pool2d(net, [int(net.shape[1]), int(net.shape[2])], scope='avg_pool')
        result.append(net)
