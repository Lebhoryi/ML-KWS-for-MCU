...
import input_data
import utils.models_official as model
...
def run_quant_inference(wanted_words, sample_rate, clip_duration_ms,
                           ...
                           model_architecture, model_size_info, output):
  ...
  predicted_indices = tf.argmax(logits[-1], 1)  # 返回的是logits中的最大值的索引号
  ...
  for i, v in enumerate(tf.trainable_variables()):
    ...
    # print(var_name[:-2])
    # 获取量化之前的weight
    with open(output+'/'+'name.txt','a') as f:
      f.write(var_name[:-2])
      f.write('\n')

    with open(output+'/'+'without_quant.h','a') as f:
      f.write(var_name[:-2])
      f.write('\n')
      var_values.tofile(f,sep=", ",format="%f")
      f.write('\n')
    ...
    #### update 2020/05/26 #########
    # dec_bits = dec_bits if dec_bits <=10 else 10

    # dec_bits = dec[i-1] + 1 if i == 1 and dec_bits > dec[i-1]+1 else dec_bits
    # dec_bits = 6 if i%2 == 1 else dec_bits
    ...
    with open(output+'/'+'with_quant.h','a') as f:
    ...
    # save dec_bits
    with open(output+'/'+'var_values.txt','a') as f:
        f.write(var_name+' number of wts/bias: '+str(var_values.shape)+ \
                ' dec bits: '+str(dec_bits)+ \
                ' max: ('+str(var_values.max())+','+str(max_value)+')'+ \
                ' min: ('+str(var_values.min())+','+str(min_value)+')')
        f.write('\n')

    # add dec_bits to excel

    if i % 2 == 0:
        sheet.write(i//2+1, 0, dec_bits)
    else:
        sheet.write(i//2+1, 1, dec_bits)
