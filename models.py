...
def create_single_fc_model(fingerprint_input, model_settings, is_training):
  ...
  weights = tf.Variable(
      tf.truncated_normal([fingerprint_size, label_count], stddev=0.01))
...
def  create_ds_cnn_model(fingerprint_input, model_settings, model_size_info,
  ...
  def _depthwise_separable_conv(inputs,
    ...
    depthwise_conv = slim.separable_convolution2d(inputs,
                                                  num_outputs=None,
                                                  stride=stride,
                                                  depth_multiplier=1, # 总共的输出通道数将为：num_filters_in * depth_multiplier
        ...
        for layer_no in range(0,num_layers):
          if layer_no==0:
            net = slim.convolution2d(fingerprint_4d, conv_feat[layer_no],
                      [conv_kt[layer_no], conv_kf[layer_no]], stride=[conv_st[layer_no], conv_sf[layer_no]],
